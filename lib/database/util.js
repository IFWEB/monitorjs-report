 ...
 * @method remove(opt [,callback]) 数据库插入
        @param [opt.onlyOne] {Boolean} 是否只删除查找结果的第一个。true-只删除一个；false-全部删除 default:false
 * @method update(query, opt [,callback]) 数据库插入
        @param [onlyOne] {Boolean} 是否只修改匹配到的第一个数据。true-只修改第一个；false-全部修改 default:false
 * @method find(opt [,callback]) 数据库查询
        @param [fields] {String} 指定返回那些内容，使用空格分开 default:'',表示返回文档所有属性。
        ...
        @param [maxAll] {String} 查询该字段最大的项的集合,返回一个数组 default:''
        @param [min] {String} 查询该字段最小的一个,如果有值，则返回一个单一对象 default:''
        @param [minAll] {String} 查询该字段最小的项的集合,返回一个数组 default:''
...
@param [onlyOne] {Boolean} 是否只删除查找结果的第一个。true-只删除一个；false-全部删除 default:false
*/
...
@param [onlyOne] {Boolean} 是否只修改匹配到的第一个数据。true-只修改第一个；false-全部修改 default:false
*/
function update(query, opt, callback){
        var self = this,
        results,
        onlyOne = opt.onlyOne,
        method = opt.onlyOne? 'findOneAndUpdate': 'updateMany';
    delete opt.onlyOne;
    results = self[method](query, opt, function(err){
        if(err){
